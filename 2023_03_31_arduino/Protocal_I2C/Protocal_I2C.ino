...
// 슬레이브에서 받는 데이터
int receiveData[8] = {0,};        // SLAVE에서 받는 버퍼

uint8_t send_buffer[9] = {0,};    // pc로 송신할 버퍼
...
//  MsTimer2::set(400, send_buff); // 500ms period
//  MsTimer2::start();
  
  MsTimer2::set(500, receive_buff); // 500ms period
  MsTimer2::start();
}
...
{ 
    ...
    send_ADC_val.data = adc;

    send_buffer[0] = 174;    // Aurix 주소
    send_buffer[1] = 184;    // PC 주소
    send_buffer[2] = 'i';    // 프로토콜의 종류를 나타내는 값
    send_buffer[3] = 3;      // 프로토콜의 데이터 번호를 나타내는 값
    send_buffer[4] = receiveData[0];    // 데이터 값
    send_buffer[5] = ADC_val.a[0]; // ADC 측정값의 하위 바이트
    send_buffer[6] = ADC_val.a[1]; // ADC 측정값의 상위 바이트

    crc_16_val_S.check_sum = CRC16_MODBUS(send_buffer, 7); // CRC16 체크섬 계산

    send_buffer[7] = crc_16_val_S.b[0]; // CRC16 체크섬의 하위 바이트
    send_buffer[8] = crc_16_val_S.b[1]; // CRC16 체크섬의 상위 바이트

    
    for(int i = 0; i < 9; i++){
      mychar = send_buffer[i];
