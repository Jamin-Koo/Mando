#include <NewPing.h>
#define L_InPin_1  12  //왼쪽모터 핀1  Right
#define L_InPin_2  11  //왼쪽모터 핀2   Right
#define R_InPin_1  10  //오른쪽모터 핀1 Left
#define R_InPin_2  9  //오른쪽모터 핀2 Left
#define L_en  13 //왼쪽모터 속도 제어 핀
#define R_en  8 //오른쪽모터 속도 제어 핀
#define F_Sonar  5  //정면부 초음파 센서 연결 핀
#define R_Sonar  4 //오른쪽 초음파 센서 연결 핀
#define L_Sonar  6 //왼쪽 초음파 센서 연결 핀
...
void setup() {
  ...
  analogWrite(L_en,110);  //왼쪽모터 속도 조정
...
int wall_fillowing_one_upgreat(){ //상황에 맞춰 한쪽 벽을 타는 함수
  ...
  if(F_distance>turn_distance && ((R_distance-50 <= L_distance && R_distance+50 >= L_distance) || R_distance >= 1500)){
    int R_speed=110-L_Error*5;
    int L_speed=110+L_Error*5;
    ...
    int R_speed=110+R_Error*5;
    int L_speed=110-R_Error*5;
    if(R_speed >= 240){ R_speed=240;}
    if(R_speed <= 100){ R_speed=100;}
    if(L_speed >= 240){ L_speed=240;}
    if(L_speed <= 100){ L_speed=100;}
...
void wall_fillowing_two_upgreat(){ //한쪽 벽을 타지 않을시 양쪽 벽을 타는 함수(조건 벽이 2개 모두 존재하며 가운데로 50mm오차 내에 존재하지 않을 경우 작동
  if((wall_fillowing_one_upgreat()) == 0){
    int R_speed=110-(R_distance-L_distance)*2;
    int L_speed=110+(R_distance-L_distance)*2;
    if(R_speed >= 240){ R_speed=240;}
    if(R_speed <= 100){ R_speed=100;}
    if(L_speed >= 240){ L_speed=240;}
    if(L_speed <= 100){ L_speed=100;}
...
void loop() {
  ...
  Serial.print(F_sensor.ping_cm()); // Send ping, get distance in cm and print result (0 = outside set distance range)
  Serial.println("cm");

  Serial.print("R_sensor: ");
  Serial.print(R_sensor.ping_cm()); // Send ping, get distance in cm and print result (0 = outside set distance range)
  Serial.println("cm");
  
  Serial.print("L_sensor: ");
  Serial.print(L_sensor.ping_cm()); // Send ping, get distance in cm and print result (0 = outside set distance range)
  Serial.println("cm");
