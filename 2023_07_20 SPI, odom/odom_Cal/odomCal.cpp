...
void odometry_calculate(void)
{
	...
	encoder.delta = encoder.frontdata - encoder.frontdata_old; //encoder.delta
	...
	baselink.l = encoder.delta / distance_ratio; //호의 길이(l) //m단위 변환
	...
	if (delta_yaw < 0)
	{
		baselink.delta_y = -(radius  - radius * cos(delta_yaw_d))
