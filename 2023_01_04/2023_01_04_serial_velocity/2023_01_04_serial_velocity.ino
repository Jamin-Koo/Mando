...
int feedforward(int pwm) {
    pwm = pwm > velocity_mm ? velocity_mm : pwm ;
}
...
void loop() {
  ...
  if (Serial.available() > 0) 
  {
     ...
     if (check_sp == 'n') 
    {
      ...
      for (int i = 0; i < 3; i++) 
      {
        data_buffer_velo[i] = Serial.read();
      }
      Serial.write(data_buffer_velo[0]);
      Serial.write(data_buffer_velo[1]);
      Serial.write(data_buffer_velo[2]);

      s=String(data_buffer);
      data = s.toInt();
      
      s2=String(data_buffer_velo);
      data2 = s2.toInt();
      
      if(motor_direction =='r') data = -data;
      if(motor_direction =='r') data2 = -data2;
      Serial.print("data1 : ");      
      Serial.print(data);
      Serial.print(", data2 ");      
      ...
      Serial.print(", velocity_mm : ");      
      Serial.println(velocity_mm);
     }
