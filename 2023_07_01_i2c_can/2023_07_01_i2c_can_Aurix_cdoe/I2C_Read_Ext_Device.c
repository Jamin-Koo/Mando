...
int rx_buff(void)
{
    ...
    for(int i = 0; i<7; i++){
        uart_rx_buff[i] = g_macAddr[i];
    }
    for(int i = 0; i<3; i++){
            uart_rx_buff2[i] = uart_rx_buff[i+1];
   }

    cnt0++;
    if((uart_rx_buff[0] == 0x73) && (uart_rx_buff[6] == 0x65) )
    {

        cnt1++;

            crc_16_val_R.check_sum = CRC16_MODBUS(uart_rx_buff2, 3);

            if((  crc_16_val_R.b[0] == uart_rx_buff[4]) && (  crc_16_val_R.b[1] == uart_rx_buff[5]))
            {
                cnt2++;
                mode_select = uart_rx_buff[1];
                input_data.data[0] = uart_rx_buff[2];
                input_data.data[1] = uart_rx_buff[3];
            }
    }
