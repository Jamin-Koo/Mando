...
// data_buffer
uint8_t INPUT_buff_NUM[3] = {0,};        // SLAVE에서 받는 버퍼

unsigned char INPUT_buff[6] = {0,};
...
void setup() {
  ...
  MsTimer2::set(10, RX_Interrupt); // 500ms period
  MsTimer2::start();
}
...
void RX_Interrupt(){
      ...
      if(mySerial.available()>0){
        ...
        //startBit = mySerial.read();    //start bit check
        //if(startBit == 's'){
          ...
          for(int i = 0; i<3; i++){
            INPUT_buff[i] = mySerial.read();
            INPUT_buff_NUM[i] = int(INPUT_buff[i]);
          }
          
          crc_16_val_S.check_sum = CRC16_MODBUS(INPUT_buff_NUM, 3); // CRC16 체크섬 계산
  
          INPUT_buff[3] = crc_16_val_S.b[0]; // CRC16 체크섬의 하위 바이트
          INPUT_buff[4] = crc_16_val_S.b[1]; // CRC16 체크섬의 상위 바이트

          INPUT_buff[5] = "e";
          
          for(int i = 0; i<6; i++){
            Serial.print(INPUT_buff[i]);
            Serial.print(", ");
          }
          Serial.println();
          for(int i = 0; i<3; i++){
            Serial.print(INPUT_buff_NUM[i]);
            Serial.print(", ");
          }
          Serial.println();
        //}
